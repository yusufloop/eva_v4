...
function generateTable($pdo) {
    ...
    if (!empty($users[0])) {
        ...
        echo '<a href="functions/delete_eva_device.php?serialNo=' . urlencode($user['SerialNoFK']) . '" title="Delete" onclick="return confirm(\'Are you sure you want to delete this device?\');">🗑️</a>';
        echo '</div>';
        echo '</td>';
            echo '</tr>';
        }
